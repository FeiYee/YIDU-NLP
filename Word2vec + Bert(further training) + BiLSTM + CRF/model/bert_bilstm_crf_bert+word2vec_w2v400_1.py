...
nn.Module
#注意pretrain_output_size这个参数，是bert的维度加上word2vec，这里bert-base的维度是768，word2vec维度是400，所以是1168，规范的话可以分开写
class bert_bilstm_crf(base_model):
    def __init__(self, pretrain_model_path = None, pretrain_output_size = 1168, lstm_hidden_size = 384,
        ...
        self.w2v_emb = pickle.load(open("./word2vec/w2v.pkl", "rb"))#读取之前训练的word2vec向量，
...
#         self.crf = CRF(lable_num)
    ...
    @torch.no_grad()
    def decode(self, dev_x, max_len = None, sen_len = None, use_cuda = None, dev_y = None, dev_z=None):
        dev_x = torch.tensor(dev_x, dtype=torch.long).to(self.device)
        dev_z = torch.tensor(dev_z, dtype=torch.long).to(self.device)
        output = self.forward(x=dev_x, z=dev_z)
        loss = None
        if dev_y != None:
