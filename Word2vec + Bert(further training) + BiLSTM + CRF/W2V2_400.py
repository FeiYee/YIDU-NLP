...
#        w2v.append(model[vocab])
#    except:
#        w2v.append(np.random.normal(0, 0.1, emb_size))#训练语料里未出现随机初始化
for vocab in vocabs:
    w2v.append(np.random.normal(0, 0.1, emb_size))

vocab = vocab.replace("\n", "")
w2v = np.array(w2v, np.float32)#转成numpy array格式
#pickle.dump(w2v, open(w2v_path + "w2v_nocls.pkl", "wb"))#保存到本地
pickle.dump(w2v, open(w2v_path + "w2v.pkl", "wb"))#保存到本地
